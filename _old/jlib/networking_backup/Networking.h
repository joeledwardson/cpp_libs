...
namespace BNP
{
	...
	struct Update
	{
		...
		Update( const char * completeBuf, uint32_t len, bool isForceSend = false );
		~Update();
	...
	struct IPContact
	{
		...
		std::vector<Update>futureUpdates;
		...
		int FindUpdate( byte updateID, UpdateList_Typ listType );
	...
	protected:
		...
		bool SendUpdates( IPContact* list, UINT size , bool forceSend);
		bool SendUpdateAccepts( IPContact* list, UINT size );
		bool SendHeartBeats(  IPContact* list,  UINT size );
		bool IsUpdateSent( IPContact* list,  UINT size, UINT uIndex);
